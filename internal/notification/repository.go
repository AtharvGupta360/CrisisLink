...
// notification. It's a outbox.TxFunc (not a plain method) so it can only run inside the
// inbox's transaction, alongside the dedup marker — that's what makes processing
// exactly-once.
func NotificationWriter(eventID int64, eventType, message string) outbox.TxFunc {
...
func (r *NotificationRepository) ListRecent(ctx context.Context, limit int) ([]Notification, error) {
