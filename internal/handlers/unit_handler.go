...
func (h *UnitHandler) Create(c *gin.Context) {
	...
	u, err := h.svc.Create(c.Request.Context(), service.CreateUnitInput{
	...
	if err != nil {
		switch {
		case errors.Is(err, service.ErrDuplicateCallSign):
			common.Error(c, http.StatusConflict, "call sign already exists", "DUPLICATE_CALL_SIGN")
		case errors.Is(err, service.ErrInvalidUnitType) || errors.Is(err, service.ErrInvalidCoordinates):
			common.Error(c, http.StatusBadRequest, err.Error(), "VALIDATION_ERROR")
		default:
...
func (h *UnitHandler) List(c *gin.Context) {
	...
	if err != nil {
		if errors.Is(err, service.ErrInvalidUnitStatus) || errors.Is(err, service.ErrInvalidUnitType) {
			common.Error(c, http.StatusBadRequest, err.Error(), "VALIDATION_ERROR")
			return
		}
...
func (h *UnitHandler) GetByID(c *gin.Context) {
	...
	if err != nil {
		if errors.Is(err, service.ErrUnitNotFound) {
...
func (h *UnitHandler) UpdateStatus(c *gin.Context) {
	...
	if err != nil {
		...
		case errors.Is(err, service.ErrUnitNotFound):
			common.Error(c, http.StatusNotFound, "unit not found", "NOT_FOUND")
		case errors.Is(err, service.ErrInvalidUnitStatus):
			common.Error(c, http.StatusBadRequest, err.Error(), "VALIDATION_ERROR")
		default:
